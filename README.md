...
| 角色 | 成员 | 负责层 | 拥有页面 | 主要职责 |
|---|---|---|---|---|
| R1 | 张恩 | 数据获取层 | 无（纯函数） | URL 解析、GitHub API、Token 存储、Mock 数据 |
| R2 | 王靖壹 | 分析引擎层 | 无（纯逻辑） | 文件检测、README 启发式分析、评分、建议、报告字符串生成 |
| R3 | 倪子宸 | 脚手架 / 路由 + 共享组件 | 无（基础设施） | 项目搭建、路由常量、通用组件、结果缓存 |
| R4 | 杨天阳 | 主流程页面 + 集成点 | 首页、检测结果页 | 仓库输入、检测主流程、结果展示、错误态处理 |
| R5 | 苏和 | 报告 / 历史 / Token 页 + 历史存储 | 报告页、历史记录页、Token 配置页 | Markdown 报告展示、历史记录、Token 配置、localStorage 工具 |
